...
- `sheet` 가 html, htm, vue, svelte 도 읽는다. 태그 사이 글과 속성값을 JSX 와 같은 길로 뽑고 `<!-- -->` 주석은 걷어낸다.
  같은 줄에서 인라인 태그만 사이에 둔 글은 한 마디로 잇고 (`붙여 넣으면 <strong>고칠 곳</strong>이 보입니다.`), 마크업
  파일의 여러 줄 문단은 줄마다 한 마디다. `<script>`, `<style>` 안은 코드로 본다.
- screenSentence 의 진행·실패·요구·완료 무늬에 고침 제안이 붙는다 (`요청을 완료하지 못했습니다` → `요청 완료 실패`).
  사전 항목의 `to` 규칙이 문장 전체를 낱말로 다시 쓰고, 표의 고침 칸이 그 제안으로 미리 채워진다.
- 표의 지적 칸이 규칙마다 한 번만 이유를 쓰고 같은 규칙의 나머지는 단서만 `(또 …)` 로 잇는다.
- 저장소 모드에 `저장소에 쓰기` 가 생겼다. 고침을 적은 파일마다 Contents 쓰기 토큰으로 같은 브랜치에 commit 하나를 만든다.
  CLI 의 `sheet apply` 와 같은 `applyRows` 를 쓰며 (hanlint 와 npm 공개 표면에 더했다), 토큰은 메모리에만 둔다.
...
- 사전 규칙 다섯 (translationese, hardWord, cliche, redundantPair, japaneseLoan) 이 그 종류의 문서 `usageShare` (기본 0.9)
  이상에 나오는 항목을 짚지 않는다. 빈도표의 `patterns` 절이 사전 항목마다 문서 수를 들고, 사업보고서 3,193편에서
  `에 대한` 등 여덟 항목과 `상기`, `를 가지고 있` 이 93~100% 에 나왔다. 실측 (1,000편): report 프리셋의 translationese
  지적이 1,000문장당 290.9건에서 9.0건으로, 전체 지적이 667.6건에서 371.8건으로 준다. 설정으로 더한 항목은 늘 짚는다.
- `hanlint usage "낱말 …" --kind report --limit 5` (두 판). 그 종류의 실제 글에서 낱말이 쓰인 문장을 BM25 순서로
  ...
  알리고 2 로 끝난다. 스킬의 4단계가 막힌 자리에서 부른다. 색인은 토큰 표를 통째로 읽지 않고 디스크에서 이분 탐색한다
  (3,193편 1,056,316문장 색인에서 조회 node 0.5초).
- 용례가 고침을 낫게 한다는 실측은 없다. 300쌍을 두 모델과 세 조건으로 재고 근거는 말뭉치 절반에서 주고 채점은 다른
  절반으로 한 실험에서, 돌리기 전에 못박은 조건 셋 (부호검정 p < 0.01, 차이 ≥ +0.05, 새 error 늘지 않음) 을 하나도
  넘지 못했다 (강한 모델 p 1.0000, 차 +0.009. `tests/_attempts/usageLift`). 같은 실행에서 낯선 낱말 결합의 비율도
  쟀는데 사람이 쓴 보고서 문장이 22.1%, AI 고침이 26~27% 라 그것으로도 AI 글을 가려내지 못한다. 용례 엔진의 확인된
  값은 규칙 쪽 지적 감소다.
- `hanlint usage` 가 낱말마다 **함께 쓰는 말** (바로 뒤의 용언, 뒤와 앞의 명사, 문서 수) 을 문장보다 먼저 낸다
  (`--format json` 의 `words`). 표를 따로 두지 않고 조회 때 postings 앞 5,000문장을 읽어 센다. `hanlint usage kinds` 가
  기계에 있는 색인을 센다. `write-korean` 과 `use-hanlint` 스킬이 묻는 자리에서 이 명령을 가리킨다.
- 색인 만들기가 문서를 한 번만 읽으며 postings 를 조각으로 내려 병합한다 (format 3, 결과 바이트는 한 번에 만든 것과
  같다). jsonl (줄 하나가 문서 하나) 을 읽어 위키백과 규모의 말뭉치를 만든다. `scripts/fetch/koWikipedia.py` 가 한국어
  위키백과 덤프를 jsonl 로 떨군다 (CC BY-SA 4.0).
...
- 화면 규칙 (screenNarration, screenSentence, screenTone, screenWord) 의 지적 문장 끝 출처가 `(화면 낱말 규약)` 이다. 사적
  프로젝트 이름이 공개 제품의 메시지와 `explain` 에 나오지 않는다. 실측 출처는 규칙 docstring 이 소유한다.
- 브라우저의 검사 워커가 시간 초과나 오류 뒤 그 요청만 끝내고 스스로 다시 뜬다. 새로고침 전까지 편집기가 멎던 것을 바꿨다.
...
- 소스의 글 추출이 식 안의 JSX (`{ok ? <span>없음</span> : x}`), 식 안의 배열 (`{[['기준월 실적', a]]}`), 백틱 안의 백틱
  (`${a ? `해마다 ${b}` : c}`) 을 바르게 읽는다. 바깥 글은 식을 비운 채 내고, 식 안의 따옴표 글과 태그 사이 글은 제 자리
  (칸) 로 따로 낸다. Taxly 에서 코드가 글로 새던 5건이 0건이 됐다.

## [0.0.16] - 2026-09-18

### Added

- 규칙 `screenWord`: 화면의 낱말. 개발 어휘 (메타데이터, 컴포넌트, 트랜잭션) 는 기본 사전이 잡고, 프로젝트가 한
  이름으로 정한 낱말 (데이터 → 자료) 은 설정의 `dictionary.screenWord` 로 더한다. 같은 자리는 설정 항목이 이긴다.
- `hanlint sheet` 가 글에 지적이 하나이고 그 지적에 고침이 있으면 고침 칸에 미리 적는다. `sheet apply` 로 바로
  되돌려 쓴다.
...
- `hanlint sheet` 의 자리가 `경로:줄:칸` 이다. `apply` 는 그 칸의 글을 바꾸므로 같은 글이 한 줄에 둘이어도 자리가 하나다.
  JSON 에도 `column` 이 있다.

### Fixed

- 규칙 등록부의 `loadAll` 이 검사마다 규칙 폴더를 다시 걷던 것을 한 번만 걷게 했다. 글 마디 2,189개의 sheet 가
  15.8초에서 4초 안으로 준다.
- `sheet apply` 가 한 줄의 고침 여럿을 뒤 칸부터 바꾼다. 앞 칸을 먼저 바꾸면 길이가 달라져 뒤 칸이
  "그 칸에 그 글이 없다" 로 실패하던 것이다 (Taxly 의 426건 가운데 47건).
...
- `sourceLiterals` 가 비교 (`page >= count`) 의 `>` 를 JSX 태그 끝으로 읽어 그 뒤 코드를 글로 잡던 것을 고쳤다.
  태그 이름, 속성 따옴표, 식의 `}` 뒤에 오고 `=` 가 뒤따르지 않는 `>` 만 태그 끝이다.
...
- 랜딩을 히어로, 원문·결과 두 칸, 아래 수정본 입력으로 다시 구성했다. 색, 심볼과 글꼴을 바꾸고
  같은 문장의 지적을 묶어 이유와 본보기를 바로 보여 준다. 수정본에는 원문을 미리 담고 기록은 한 번
  눌러 저장한다. 원문 보호와 고침 후보는 수정 후 나타난다. 다크·라이트 전환과 선택 복원을 제공하며
  심볼은 ㅎ과 고침 표시를 결합했다. 기록과 승인, 개인 GitHub 보관의 경계는 유지한다.
...
- **`hanlint primer` 가 쓰기 전에 읽는 한 장을 낸다.** 프리셋에서 켜진 규칙마다 기술서의 고치기 절과 본보기
  ...
  프리셋과 문체에서도 100줄 안이다. 후는 게이트가 error 0 을 보장하고 Python 과 npm 이 글자 단위로 같다
- **대화용 프리셋 `chat` 을 더했다.** 답변 하나에 맞지 않는 글의 짜임 규칙 (도입, 제목, 절, 문단 나누기, 사실
  나열, 코드 블록 대조, 두 자리 대조) 열여섯과, 견줄 답변 말뭉치가 없어 어차피 침묵하는 outsideProfile 을 끈다.
  견줄 프로파일은 없다
...
- **한 번도 안 울리던 규칙 넷의 버그.** 기준 말뭉치에서 0건이라 아무도 못 보던 자리다. `tableOddCell`
  ...
  문장이 우연히 계사문이면 오탐이 났다. `installImport` 의 설치 줄 캡처가 백틱에서 안 멈춰
  `pip install kubernetes` 뒤의 산문에서 `python`, `client`, `library` 를 설치된 패키지로 등록하고 그
  이름을 import 하는 코드를 조용히 통과시켰다 (정탐을 죽이는 쪽 버그다). `keywordHeading` 이 제목만
  봐서 **제목엔 없고 첫 문단에만 있는 검색어**가 `keywordMissing` 과의 사이로 빠졌다. 이제 두 규칙이
  같은 자리 (제목 + 첫 문단) 를 본다
...
- **em 대시 강행규칙이 제품 파일 118개를 안 보고 있었다.** 게이트 `testDash` 가 `.js` 와 `.html` 을
  ...
  614개를 본다 (나머지는 바이너리와 TSV). 지금 실제 위반은 0건이다
- **`--config` 를 슬래시 경로로 주고 설정이 작업 폴더 밖이면 두 판의 첫 줄과 JSON `config` 값이 갈렸다.**
  파이썬은 `str(Path)` 로 역슬래시를 내고 npm 은 받은 문자열을 그대로 들었다. 설정 출처는 이제 언제나
  `/` 로 적는다. 파리티 게이트가 늘 `tmp_path` 의 역슬래시만 넘겨 이 자리를 못 봤어서 작업 폴더 밖
  사례를 넣었다
- **두 판의 서브명령 집합을 견주는 게이트가 없었다.** npm 은 `COMMANDS` 아홉과 `PYTHON_ONLY` 열넷을
  손으로 들고 있는데 파이썬 서브파서와 어긋나도 아무도 몰랐다. 한쪽에만 있는 이름을 양방향으로 잡는다
...
- **annotated tag와 GitHub Release가 같은 검토본을 쓴다.** 정확한 Git 범위와 버전 CHANGELOG를 대조한
  한 메시지 파일로 태그를 만들고, 배포 워크플로가 태그 객체의 본문을 그대로 공개한다. 태그를 만들 때
  `--cleanup=verbatim`을 써서 Markdown 제목이 Git 주석으로 빠지지 않게 한다
...
- **오탐이 많던 규칙 넷을 종류로 좁혔다.** 규칙을 지우지 않고 그 규칙이 맞지 않는 종류에서만 끈다.
  ...
  자습서는 명령 전후 상태를 견주려 같은 출력을 일부러 다시 둔다). `headingUniform` 은 어미를 한글
  음절로 제한하고 (한자 독음 괄호 `]` 와 라틴 약어 `P` 로 끝난 제목 셋이 빠졌다) severity 를 notice 로
  낮춘 뒤 `docs` 와 `guide` 에서 끈다 (`생성하기 / 적용하기 / 검증하기` 는 태스크 문서의 관례다).
  `introLong` 은 본문 절이 하나뿐이거나 도입이 문단의 과반이면 넘어가고 (위키뉴스 단신은 부록 H2 하나
  때문에 기사 전체가 도입으로 세어졌고 제목 없는 수필은 도입이 78문단이었다) `encyclopedia` 에서
  끈다 (백과 머리말은 길게 정의와 요약을 담는 것이 관례다). 넷 다 규칙이 온 자리인 blog 에서는 켠 채 둔다
- **`platformApi` 사전에서 `/proc/` 를 뺐다.** 발화 여덟 건이 전부 이 한 항목이었고 전부 쿠버네티스
  안내서의 `kubectl exec` 와 컨테이너 안 셸 기록이라 독자가 멈추지 않는다. 코드에 박히는 API 호출
  (`os.startfile(`, `peak_wset`) 은 그대로 잡는다. 규칙이 아니라 사전 한 줄의 문제였다
...
- **평가 화면의 판정 논리를 파일로 빼서 브라우저 없이 잰다.** 휠에 실려 나가는
  `panelReviewPage.html` 의 인라인 스크립트 안에 있어 아무도 못 재던 논리다. 게이트 다섯은 정체성
  누출과 변조만 봤고 content 우선 잠금, 목소리 기권, 진행 저장과 재개, 내보내기 계약은 브라우저를
  열어야만 확인할 수 있었다. 순수 함수만 `data/panelReviewPage.js` 로 빼고 `renderPanelReviewHtml` 이
  렌더할 때 인라인한다 (단일 HTML, 외부 요청 0, CSP 계약은 그대로다). `node --test` 가 같은 파일을
  읽어 열한 가지를 잰다. 다른 배정과 꼴이 틀린 저장분을 거부하는 것, 자리 번호를 범위 안으로 잡는 것,
  이유의 자리표시자를 안 채운 것으로 세는 것까지 본다. 새 의존성은 없다
