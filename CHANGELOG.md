...
- 용례가 고침을 낫게 한다는 실측은 없다. 같은 수정 과제 45쌍을 용례 있이/없이 시킨 두 표본 (모델 둘, 조건 셋) 에서
  규칙 해결은 강한 모델이 어느 조건이든 천장이었고 낱말 보존은 회차마다 방향이 뒤집혔다. 문서 어디에도 효과를 적지
  않는다 (`tests/_attempts/usageLift`). 용례 엔진의 확인된 값은 규칙 쪽 지적 감소다.
