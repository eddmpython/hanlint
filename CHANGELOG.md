...
- **조사 맞추기가 숫자를 안 읽었다.** `한글이 아니면 손대지 않는다` 로 로마자와 숫자를 함께 뺐는데
  숫자는 읽는 법이 정해져 있다. 이제 한자어로 읽어 센다 (`10` 은 십이라 `10으로`, `2024` 는
  이천이십사라 `2024로`). 끝자리만 보면 `20` 을 영으로 읽어 틀린다. 로마자만 남기고 그 자리는
  발음에 달려 세어서 확정되지 않는다는 것을 근거로 적었다
