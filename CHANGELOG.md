...
- **종류별 프로파일과 `outsideProfile`.** 글의 종류 (프리셋) 마다 같은 종류의 편집된 글 수만 문장에서 문장 길이, 쉼표,
  ...
  "문장 길이 41어절. 기술 문서 27,794문장 가운데 상위 0.4%" 같은 사실로 짚는다. 점수는 없다. `hanlint profile build`
...
- **기준 말뭉치 1,605편, 일곱 종류.** 한국어 위키백과의 알찬 글과 좋은 글 (백과), 위키문헌 소설, MDN 한국어 (기술 문서) 를
  더하고 Kubernetes 와 위키뉴스와 위키문헌의 한도를 풀었다. 문장 147,380개. 지문 표 (`scripts/buildPrints.py`, Parquet) 와
