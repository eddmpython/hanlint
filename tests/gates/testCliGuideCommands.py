"""명령 안내의 명령 표가 실제 두 판의 명령 목록과 맞는지 본다.

명령 안내는 명령마다 npm 칸에 예 또는 아니오를 적는다. 이 게이트가 그 칸을 실제 실행으로 확인하므로
...
def testTableCoversEveryPythonCommand():
    ...
    assert not commands - listed, f"명령 안내 명령 표에 없는 명령: {sorted(commands - listed)}"
...
def testNpmColumnMatchesNpmCli():
    ...
    for command, claimed in sorted({(command, claimed) for command, claimed in rows}):
        ...
        if claimed == missing:
            wrong.append(f"{command}: 명령 안내 는 npm {'있다' if claimed else '없다'} 라는데 실제는 반대다")
    assert not wrong, "\n".join(wrong)
