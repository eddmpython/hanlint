...
def sectionResult(doc: DocumentPrint, config: Config) -> Iterator[Finding]:
    """산문만 있고 눈으로 볼 결과 (코드, 표, 그림, 파일 이름, 출력 문장) 가 없는 본문 절.
    ...
    안 잡는 것: 도입과 마지막 절 (결말은 확인으로 닫는다). 짧은 절. 코드나 표나 그림이 있는 절. 파일 이름이나
        출력을 말하는 문장이 있는 절. 참고 문서라 결과가 원래 없으면 이 규칙을 끈다. notice 로만 낸다.
    """
