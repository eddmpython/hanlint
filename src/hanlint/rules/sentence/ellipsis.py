...
def ellipsis(doc: DocumentPrint, config: Config) -> Iterator[Finding]:
    ...
    대괄호만 예외로 두어 그 사이에 큰따옴표가 끼면 예외가 풀렸다). 영숫자에 붙은 점 셋 (`v0.0.1...HEAD`
    같은 범위 표기. CHANGELOG 의 compare URL 이 실측 사례다). 산문만 본다.
    """
    for sentence in doc.sentences:
        found = [m for m in ELLIPSIS.finditer(sentence.text) if not insideAny(m.start(), m.end(), sentence.quoted)]
        if found:
            yield Finding(
                "ellipsis",
                sentence.line,
