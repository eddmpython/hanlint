...
def renderLearnJson(candidates: tuple[LearnedExemplar, ...], beforePath: Path, afterPath: Path) -> str:
    data = {
        "version": 1,
...
def run(args: argparse.Namespace) -> int:
    ...
    if args.format == "json":
        rendered = renderLearnJson(candidates, args.before, args.after)
    elif args.format == "toml":
        rendered = renderLearnToml(candidates)
    else:
        rendered = renderLearnText(candidates, args.before, args.after)
