...
try {
  ...
  assert.equal(await evaluate(`JSON.parse(localStorage.getItem(${storageKey})).records.length`), 1);
  ...
  await waitFor(`JSON.parse(localStorage.getItem(${storageKey})).draft.original === '원고가 저장되어집니다.' && !document.querySelector('#fixAllButton').disabled`);
  await click("바로 고치기");
  await waitFor("document.querySelector('#draft').value === '원고가 저장됩니다.'");
  assert.equal(await evaluate(`JSON.parse(localStorage.getItem(${storageKey})).draft.original`), "원고가 저장되어집니다.");
