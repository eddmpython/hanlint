...
4. `error` 를 전부 고친다. **후보가 있으면 후보부터 읽되 기계가 골랐다고 여기지 않는다.** 이중 피동은
   활용과 시제를 보존한 문장, 긴 문장은 끊어 볼 연결 어미 자리, 가리킬 것 없는 지시어는 앞 문장의 명사들이
   나온다. 문맥에 맞는 것이 없으면 전부 버리고 뜻에 맞는 이름을 직접 쓴다. 후보가 없거나 다시 쓸 폭이 더
   넓으면 본보기를 본다. 사람이 읽는 꼴은 글 끝에 규칙마다 한 줄,
