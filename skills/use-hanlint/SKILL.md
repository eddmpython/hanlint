...
4. `error` 를 전부 고친다. **후보가 있으면 후보부터 읽되 기계가 골랐다고 여기지 않는다.** 긴 문장은 끊어
   ...
   `--format json` 은 지적마다 `exemplar` (`before`, `after`, `moved`) 를 든다. 목표 문장에 같은 국소
   변환이 가능할 때만 전후 차이를 본뜨고, 본보기의 이름과 숫자와 사실은 옮기지 않는다. 맞지 않으면
   원문을 두고 사람에게 확인한다. 규칙이 왜 있는지까지 알아야 하면
