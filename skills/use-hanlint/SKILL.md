...
2. `hanlint fix 글.md` 를 먼저 돌린다. 기계가 확실히 고칠 수 있는 자리 (번역투, 명령형 뒤 마침표, 이중 부정,
   인용이 아닌 이중 피동) 를 원문에 적용하고 무엇을 바꿨는지 줄마다 보여 준다. 건너뛴 자리는 이유가 붙어
   있으니 손으로 고친다.
...
4. `error` 를 전부 고친다. **후보가 있으면 후보부터 읽되 기계가 골랐다고 여기지 않는다.** 긴 문장은 끊어
   볼 연결 어미 자리, 가리킬 것 없는 지시어는 앞 문장의 명사들이 나온다. 직접 인용 안의 이중 피동은
   글쓴이의 사용이 아니므로 지적하지 않는다. 문맥에 맞는 것이 없으면 전부 버리고 뜻에 맞는
   이름을 직접 쓴다. `--format json`의 지적에 `patch`가 있으면 정규화한 원문, 규칙, 프리셋, 국소 표지,
   독자 상태가 모두 같은 승인 고침이다. 현재 문장 전체를 `patch.after`로 바꾼다. `patch`가 없으면
   `exemplar`는 규칙을 설명하는 교육 자료일 뿐 정답으로 적용하지 않는다. 지적 이유와 후보와 문형으로
   필요한 자리만 고치고, 뜻을 확정할 수 없으면 원문을 두고 사람에게 확인한다. 규칙이 왜 있는지까지 알아야 하면
