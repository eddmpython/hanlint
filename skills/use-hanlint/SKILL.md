...
4. **자리만 고친다.** 후보가 있으면 후보부터 읽되 기계가 골랐다고 여기지 않는다. 뜻에 맞는 것이 없으면 이름을
   ...
   보장된 문장 틀) 을 본다. 그 종류의 실제 글이 그 낱말을 어떻게 쓰는지 막히면 `hanlint usage "낱말 낱말" --kind report`
   가 색인된 말뭉치의 문장을 보인다 (색인이 없으면 만드는 법을 알리고 2 로 끝난다. 사용자가 만들지 정한다). 보인 문장은
   쓰임의 근거이지 정답이 아니다. 지적을 없애려고 문장이나 사실을 지우지 않는다.
