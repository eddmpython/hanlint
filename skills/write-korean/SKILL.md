...
7. 지적이 있는 규칙만 `guidance`의 본보기와 견준다. 목표 문장에 같은 국소 변환을 적용할 사실이 있을 때만
   본뜨고, 본보기의 이름과 수치와 사실은 옮기지 않는다. 맞지 않거나 사실을 더해야만 고칠 수 있으면 원문을
   두고 사람에게 확인한다. 프로젝트 본보기가 있으면 내장 본보기보다 그 조직의 승인된 고침이 선택된다.
...
- 본보기가 있다는 이유만으로 AI 수정이 더 낫다고 주장하지 않는다. `qwen3:8b` 30쌍 실측에서는 안전한
  성공이 본보기 유무 모두 12/30이었으므로, 맞지 않는 본보기는 쓰지 않는 것이 계약이다.
