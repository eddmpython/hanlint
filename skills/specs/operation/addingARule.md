...
3. **표층으로 갈리는지 정한다.** 정규식과 어절 판정 (`analysis/tokenize.py` 의 `words`, 조사와 꼬리 사전) 으로
   결정적으로 갈리는 것만 규칙이 된다. 그 문장을 찍어 실제 판정을 본 뒤 쓴다. 표층으로 못 가르면 뜻의 영역이라
   규칙을 만들지 않는다. 형태소 분석기는 없다
