...
태그는 Markdown 제목을 보존하는 정리 방식으로 그 파일을 읽고 GitHub Release도 같은 본문을 쓴다. 전체
배포 절차는 `operation.release` 가 정본이다.
...
| `pre-commit` | staged 텍스트 파일의 em 대시와 en 대시와 제어 문자, `src` `tests` `hooks` `scripts` `npm` `web` 아래 snake_case 코드 파일 이름 |
