...
화면의 글 (단추, 이름표, 상태, 빈 상태) 용이며 낱말 자리의 문장, 화면 해설, 접객 말투를 잡고 산문 규칙은 끈다.
산문 프리셋에서 화면 해설과 접객 말투만 잡으려면 `enforceStyle = ["screenNarration", "screenTone"]` 을 둔다.

| 프리셋 | 글의 목적 | 끄는 규칙 | 견주는 프로파일 |
|---|---|---:|---|
| `blog` | 독자를 부르고 절마다 결과를 남기는 글 | 3개 | 블로그 |
| `guide` | 단계별 안내서 | 5개 | 안내서 |
| `report` | 보고서 | 8개 | 보고문 |
| `essay` | 수필 | 9개 | 수필 |
| `fiction` | 소설 | 9개 | 소설 |
| `docs` | 참고 문서, 명세, README | 12개 | 기술 문서 |
| `encyclopedia` | 백과 항목 | 13개 | 백과 |
| `chat` | 대화 답변. 글의 짜임을 재는 규칙을 끈다 | 20개 | 없음 |
