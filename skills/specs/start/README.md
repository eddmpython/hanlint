---
id: start.README
title: 처음 여는 사람이 읽는 것
category: start
purpose: 이 저장소를 처음 열었을 때 먼저 읽을 문서를 순서대로 안내한다.
whenToUse:
  - 처음 왔다
  - 어디부터 읽나
  - 이 프로젝트가 뭔가
status: curated
---

# 처음 여는 사람이 읽는 것

1. [`start.hanlintSkillOs`](hanlintSkillOs.md) 이 저장소가 소유하는 것과 어디로 가야 하는지
2. [`start.product`](product.md) 무엇을 잡고 무엇을 잡지 않는가
3. [`start.cli`](cli.md) 첫 검사, 프리셋, 설정과 명령별 지원 범위
4. [`start.integrations`](integrations.md) GitHub Actions, pre-commit과 에이전트 훅 연결
5. [`start.readerContract`](readerContract.md) Contract, Finding, Patch의 공개 프로토콜

설치 없이 글을 고치려면 [브라우저 편집기](https://eddmpython.github.io/hanlint/)를 연다.
원고 보관과 고침 승인은 [사용과 저장 안내](https://eddmpython.github.io/hanlint/guide.html)에 있다.

그다음 할 일이 규칙 추가면 `operation.addingARule` 로 간다.
