...
test("applies every machine fix and the findings vanish", () => {
  ...
  assert.ok(result.text.includes("모든 분야에서 기준이 필요합니다. 파일을 확인하세요 노력해야 합니다."));
  assert.ok(result.text.includes("`에 있어서` 는"));
  const after = rulesOf(result.text);
  for (const rule of ["translationese", "imperativePeriod", "doubleNegative"]) assert.ok(!after.has(rule), rule);
});
